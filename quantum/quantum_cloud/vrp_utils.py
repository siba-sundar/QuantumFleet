...
def cluster_locations(locations: List[Dict], max_cluster_size: int = 8) -> List[List[Dict]]:
    """Pre-cluster locations to keep quantum circuits manageable using hierarchical approach"""
    if len(locations) <= max_cluster_size:
        return [locations]
    
    # For very large datasets, use a fast hierarchical approach
    if len(locations) > 100:
        return _hierarchical_cluster_fast(locations, max_cluster_size)
    else:
        return _kmeans_cluster(locations, max_cluster_size)


def _kmeans_cluster(locations: List[Dict], max_cluster_size: int) -> List[List[Dict]]:
    """KMeans clustering for medium-sized datasets"""
    ...
    n_clusters = max(2, (len(locations) + max_cluster_size - 1) // max_cluster_size)
    ...
    # Post-process to ensure max_cluster_size constraint
    final_clusters = []
    for cluster in clusters:
        if len(cluster) <= max_cluster_size:
            ...
            sub_clusters = _kmeans_cluster(cluster, max_cluster_size)
            final_clusters.extend(sub_clusters)
    
    return final_clusters


def _hierarchical_cluster_fast(locations: List[Dict], max_cluster_size: int) -> List[List[Dict]]:
    """Fast hierarchical clustering for large datasets"""
    # Sort by latitude first for spatial locality
    sorted_locs = sorted(locations, key=lambda x: (x['lat'], x['lon']))
    ...
    for loc in sorted_locs:
        if len(current_cluster) < max_cluster_size:
            ...
            # Check if this location is close to the current cluster centroid
            if current_cluster:
                centroid_lat = sum(l['lat'] for l in current_cluster) / len(current_cluster)
                centroid_lon = sum(l['lon'] for l in current_cluster) / len(current_cluster)
                
                # Distance threshold (in degrees, roughly 10km)
                distance = abs(loc['lat'] - centroid_lat) + abs(loc['lon'] - centroid_lon)
                
                if distance < 0.1:  # Close enough to current cluster
                    clusters.append(current_cluster)
                    current_cluster = [loc]
                else:
                    current_cluster.append(loc)
            
            # If cluster is still too large, force split
            if len(current_cluster) > max_cluster_size:
                clusters.append(current_cluster[:max_cluster_size])
                current_cluster = current_cluster[max_cluster_size:]
