...
def enforce_constraints(
    ...
    logger.info(f"Enforcing constraints: max_stops={max_stops}, max_dist={max_dist:.1f}km, max_time={max_time:.1f}h")
    ...
    # Adjust for vehicle capacity with iteration limit to avoid infinite loops
    changed = True
    cap_iter = 0
    max_cap_iter = 10
    while changed and cap_iter < max_cap_iter:
        changed = False
        cap_iter += 1
        for vid in allowed_vehicles:
            locs = assignments.get(vid, [])
    ...
    # Adjust for distance/time constraints with iteration limit
    AVG_SPEED_KMPH = 30.0
    change_loop = True
    dist_iter = 0
    max_dist_iter = 10
    while change_loop and dist_iter < max_dist_iter:
        change_loop = False
        dist_iter += 1
