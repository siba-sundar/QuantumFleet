...
def preprocess_to_features(raw: Dict) -> Tuple[pd.DataFrame, Dict[str, Vehicle], Dict[str, Depot]]:
    vehicles = {v["id"]: Vehicle(
        id=v["id"], capacity=float(v["capacity"]), type=v["type"],
        max_shift_hours=float(v["max_shift_hours"]), current_location=v["current_location"]
    ...
    for l in raw["locations"]:
        loc = Location(
            id=l["id"], lat=float(l["lat"]), lon=float(l["lon"]),
            demand=float(l["demand"]), priority=int(l["priority"]),
            time_window=(float(l["time_window"][0]), float(l["time_window"][1]))
        )
