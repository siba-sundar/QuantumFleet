...
export async function fetchEnhancedFleet(includeReserved = true, includeSentiment = true, businessUid = null) {
...
export async function reserveTruck(truckId, reservationId, assignDriver = false) {
...
export async function fetchDriverSentimentHistory(driverId, limit = 10) {
  const res = await fetch(`${API_BASE}/api/sentiment/driver/${driverId}?limit=${limit}`);
  if (!res.ok) {
    const error = await res.json();
...
export async function fetchDriverSentiment(driverId) {
  const res = await fetch(`${API_BASE}/api/sentiment/driver/${driverId}/current`);
  if (!res.ok) {
    const error = await res.json();
...
export async function fetchFleetSentimentStats() {
  ...
  if (!res.ok) {
    const error = await res.json();
    throw new Error(error.error || 'Failed to fetch fleet sentiment statistics');
  }
...
export async function sendTrackingLink(vehicleId, contactInfo, contactMethod = 'phone', driverName = 'Driver', customMessage = null) {
  const requestData = {
    vehicleId,
    driverName,
    customMessage,
...
export async function updatePaymentStatus(reservationId, paymentStatus, paymentDetails = {}) {
  const res = await fetch(`${API_BASE}/api/reservations/${reservationId}/payment`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function getReservationStats(businessUid = null) {
  ...
  const res = await fetch(`${API_BASE}/api/reservations/stats/summary${params}`)
...
export async function getUpcomingPickups(days = 7) {
  const res = await fetch(`${API_BASE}/api/reservations/upcoming/pickups?days=${days}`)
...
export async function updateDriverProfessionalInfo(uid, professionalInfo) {
  const res = await fetch(`${API_BASE}/api/driver-profiles/${uid}/professional`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function fetchAvailableDrivers(includeAssigned = false, includeSentiment = true) {
  const params = new URLSearchParams({
...
export async function getPerformanceData(businessUid = null, period = 'monthly') {
  const params = new URLSearchParams()
  if (businessUid) params.append('businessUid', businessUid)
