...
export async function apiCreateDelivery({ truckId, origin, destination, eta, payee, amount }) {
  return axios.post(`${BASE_URL}/delivery/create`, { truckId, origin, destination, eta, payee, amount });
}

// 2️⃣ Finalize Delivery Flow
export async function apiFinalizeDelivery(signer, orderId, payee) {
  // 1️⃣ Get PoD contract instance
...
export async function apiAddCheckpoint(orderId, latE6, lonE6, ts, status) {
  return axios.post(`${BASE_URL}/delivery/checkpoint`, { orderId, latE6, lonE6, ts, status });
}
