      ...
      if (user) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // For newly created accounts that might not have user document yet,
            // still provide basic user info to allow access
            console.warn('User document not found, providing basic auth info:', user.uid);
            callback({
              uid: user.uid,
              email: user.email,
              phoneNumber: user.phoneNumber,
              userType: null, // Will be determined by other checks
              emailVerified: user.emailVerified,
          ...
          // On error, still provide basic user info if user is authenticated
          callback({
            uid: user.uid,
            email: user.email,
            phoneNumber: user.phoneNumber,
