...
class WebSocketService {
  ...
  setupEventHandlers() {
    ...
    // Enhanced SOS emergency alert handler
    this.socket.on('emergency_sos_alert', (data) => {
      console.log('🚨🚨 EMERGENCY SOS ALERT RECEIVED:', data);
      
      // Show immediate browser notification for SOS
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('🚨 EMERGENCY SOS ALERT', {
          body: `Vehicle ${data.vehicleId}: ${data.message}`,
      ...
      try {
        ...
        audio.play().catch(() => console.log('Emergency sound not available'));
      } catch (error) {
        console.log('Emergency sound not available');
      }
      
      // Emit to components with special SOS flag
      this.emit('emergency_sos_alert', data);
      this.emit('new_alert', { ...data, isSOS: true, isEmergency: true });
    });
  ...
  /**
   * Request notification permissions for emergency alerts
   */
  ...
  subscribeToEmergencyAlerts() {
    if (!this.isConnected) {
      console.warn('WebSocket not connected. Cannot subscribe to emergency alerts.');
      return;
    }

    this.socket.emit('subscribe_emergency_alerts');
    console.log('🚨 Subscribed to emergency SOS alerts');
  }
}
