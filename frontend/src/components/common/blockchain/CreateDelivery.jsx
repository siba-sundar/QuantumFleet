...
export default function CreateDelivery({
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (!window.ethereum) throw new Error("MetaMask not detected");
      if (!truckId || !origin || !destination) throw new Error("Missing truckId/origin/destination");
      if (!eta || isNaN(Number(eta))) throw new Error("ETA must be a valid unix timestamp");
      if (!ethers.isAddress(payee)) throw new Error("Invalid payee address");
      ...
      // 1. Create Delivery on Blockchain
      const deliveryContract = new ethers.Contract(DELIVERY_ADDRESS, deliveryAbi, signer);
      const tx1 = await deliveryContract.createDelivery(truckId, origin, destination, Number(eta));
      const receipt1 = await tx1.wait();
      ...
      toast.success("Delivery created on blockchain");
      ...
      // 4. Initialize Proof of Delivery
      const podContract = new ethers.Contract(POD_ADDRESS, podAbi, signer);
      const tx3 = await podContract.initProof(orderId);
      await tx3.wait();
      toast.success("Proof of delivery initialized");

      // 5. Create Reservation in Database with blockchain orderId
      if (reservationData) {
        try {
          ...
          toast.error("Warning: Blockchain transaction successful but database update failed");
        }
      }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        } text-white rounded w-full flex items-center justify-center space-x-2`}
      >
        {loading && (
