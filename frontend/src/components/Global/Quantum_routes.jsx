...
const VehicleRoutingOptimizer = () => {
  ...
  const quantumServerUrl = import.meta.env.VITE_API_QUANTUM_SERVER || "http://localhost:8000";
  ...
  const handleDepotLocationSelect = (index, locationData) => {
    if (locationData) {
      const updated = [...depots];
      updated[index].lat = locationData.coordinates.lat;
      updated[index].lon = locationData.coordinates.lng;
      updated[index].address = locationData.address;
      updated[index].placeId = locationData.placeId;
      setDepots(updated);
    }
  };
  ...
  const ResultsDisplay = ({ results }) => {
    ...
    return (
      <div className="mt-8 bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-2xl font-bold text-[#020073] mb-6">Optimization Results</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(results.per_vehicle_summary).map(([truck, summary]) => (
              <div key={truck} className="bg-white border-2 border-blue-100 rounded-lg p-4">
                <h4 className="font-semibold text-[#020073] mb-2">{truck}</h4>
          ...
          <div className="overflow-x-auto">
            <table className="w-full bg-white border border-gray-200 rounded-lg">
              <thead className="bg-blue-50">
                <tr>
                  <th className="px-4 py-2 text-left font-semibold text-[#020073]">Location</th>
                  {Object.keys(results.counts_by_location[Object.keys(results.counts_by_location)[0]]).map(truck => (
                    <th key={truck} className="px-4 py-2 text-center font-semibold text-[#020073]">{truck}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto p-4">
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <p className="text-gray-600 text-center mb-6">Optimize delivery routes using quantum computing algorithms</p>
          ...
          <div className="mb-6 bg-blue-50 rounded-lg p-4 border border-blue-100">
            <h2 className="text-lg font-semibold text-[#020073] mb-3">Constraints</h2>
