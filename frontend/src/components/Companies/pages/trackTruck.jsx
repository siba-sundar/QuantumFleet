...
function TruckDetails() {
            ...
            if (enhancedTrucks.length > 0) {
                for (const truck of enhancedTrucks) {
                    try {
                        ...
                        // Method 1: Try to get reservation by assigned driver ID
                        ...
                        // Method 2: If no data from driver-specific API, try to get by business UID
                        if (!reservationData && user?.uid) {
                            const businessReservationResponse = await fetch(`${import.meta.env.VITE_API_BASE || 'http://localhost:4001'}/api/reservations?businessUid=${user.uid}`);
                            if (businessReservationResponse.ok) {
                                const businessResData = await businessReservationResponse.json();
                                if (businessResData.reservations && businessResData.reservations.length > 0) {
                                    // Find the reservation that matches this truck
                                    reservationData = businessResData.reservations.find(res => 
                                        res.trucks?.some(t => 
                                            t.assignedDriver?.id === truck.reservationSummary?.assignedDriver?.id ||
                                            t.truckId === truck.id ||
                                            t.truckNumber === truck.number
                                        )
                                    ) || businessResData.reservations[0]; // Fallback to first reservation
                                }
                            }
                        }
                        
                        if (reservationData && reservationData.trucks?.[0]) {
                            ...
                            Object.assign(truck, {
                                ...
                                pickupLocationData: reservationData.trucks[0].pickupLocationData,
                                dropLocationData: reservationData.trucks[0].dropLocationData,
                                // Also add customer data at truck level for easier access
                                customer: reservationData.customerInfo
                            });
                        }
                    } catch (error) {
                        // Silent error handling for reservation data
                    }
                }
            }
    ...
    const resolveDriverForTruck = async (truck) => {
        ...
        if (directUid) {
            ...
            // Load driver profile for truck details
            try {
                const profileResponse = await fetch(`${import.meta.env.VITE_API_BASE || 'http://localhost:4001'}/api/driver-profiles/${directUid}`);
                if (profileResponse.ok) {
                    const profileData = await profileResponse.json();
                    setDriverProfile(profileData.profile);
                }
            } catch (error) {
                // Silent error handling for driver profile loading
            }
        } else {
    ...
    const routeInfo = useMemo(() => {
        ...
        // Enhanced customer name resolution with multiple fallback paths
        let customerName = 'Unknown Customer';
        
        // Try multiple paths for customer name based on actual database structure
        // Priority order based on the enhanced truck model schema
        if (reservationDetails.customerInfo?.contactName) {
            customerName = reservationDetails.customerInfo.contactName;
        } else if (reservationDetails.customerInfo?.company) {
            customerName = reservationDetails.customerInfo.company;
        } else if (reservationDetails.customerName) {
            customerName = reservationDetails.customerName;
        } else if (selectedTruck.reservationSummary?.customerName) {
            customerName = selectedTruck.reservationSummary.customerName;
        } else if (reservationDetails.customerInfo?.name) {
            customerName = reservationDetails.customerInfo.name;
        } else if (reservationDetails.customerInfo?.companyName) {
            customerName = reservationDetails.customerInfo.companyName;
        } else if (selectedTruck.customer?.contactName) {
            customerName = selectedTruck.customer.contactName;
        } else if (selectedTruck.customer?.company) {
            customerName = selectedTruck.customer.company;
        } else if (selectedTruck.customer?.name) {
            customerName = selectedTruck.customer.name;
        } else if (selectedTruck.customer?.companyName) {
            customerName = selectedTruck.customer.companyName;
        }
    ...
    const handleTruckSelect = async (id) => {
        ...
        try {
            // Get fresh data directly and process it
            // Get business UID for filtering if user is a business
            const businessUid = user?.userType === 'business' ? user?.uid : null;
                        ...
                        if (!reservationData && user?.uid) {
                            ...
                            if (businessReservationResponse.ok) {
                                ...
                                if (businessResData.reservations && businessResData.reservations.length > 0) {
                                    ...
                                    ) || businessResData.reservations[0];
                                }
                            }
                        }
            ...
            // Update trucks state with enhanced data
            setTrucks(enhancedTrucks);
            
            // Find truck in fresh enhanced data
            const asTruck = enhancedTrucks.find(t => t.id === value);
            if (asTruck) {
                setSelectedTruckId(value);
                await resolveDriverForTruck(asTruck);
            } else {
                // If not found as truck ID, try to find truck by driver ID
                const driverTruck = enhancedTrucks.find(t => 
                    t?.driver?.id === value || 
                    t?.reservationSummary?.assignedDriver?.id === value
                );
                if (driverTruck) {
                    setSelectedTruckId(driverTruck.id);
                    setSelectedDriverId(value);
                    await resolveDriverForTruck(driverTruck);
                } else {
                    // Clear selections if neither truck nor driver found in current business context
                    setSelectedTruckId(null);
                    setSelectedDriverId(null);
                }
            }
        } catch (error) {
            // Silent error handling for fleet data loading
        } finally {
            setLoading(false);
        }
    };

    // Truck details for display - matches driver component logic
                ...
                <div className="p-6">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="flex items-center text-gray-600">
                                        ...
                                        <span className="font-medium">Truck:</span> {truckDetails?.licensePlate || selectedTruck?.number}
                                    </span>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        ...
                        <div className="xl:col-span-2">
                            <div className="bg-white rounded-xl shadow-lg p-6 h-[1322px]">
                                ...
                                {/* Map Container */}
                                <div className="h-[calc(100%-170px)] rounded-lg overflow-hidden">
                                ...
                                <div className="space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <p className="text-sm font-semibold text-gray-800">{truckDetails?.licensePlate || 'N/A'}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500 font-medium mb-1">Model</p>
                                            <p className="text-sm font-semibold text-gray-800">{truckDetails?.model || 'N/A'}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <p className="text-sm font-semibold text-gray-800">{truckDetails?.driver || 'Not Assigned'}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500 font-medium mb-1">Capacity</p>
                                            <p className="text-sm font-semibold text-gray-800">{truckDetails?.capacity || 'N/A'}</p>
                                        </div>
                                    </div>
                                    
                                    <div>
                                        <p className="text-xs text-gray-500 font-medium mb-1">Last Maintenance</p>
                                        <p className="text-sm font-semibold text-gray-800">{truckDetails?.lastMaintenance || 'N/A'}</p>
                                    </div>
                                </div>
                            </div>
