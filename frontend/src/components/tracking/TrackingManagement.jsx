...
const TrackingManagement = () => {
  ...
  const fetchVehicles = async () => {
    try {
      // Use enhanced fleet API to get business-specific vehicles
      const fleetResponse = await fetchEnhancedFleet(true, true);
      let businessVehicles = fleetResponse.trucks || [];
      
      // Filter vehicles by business user context
      if (user?.userType === 'business' && user?.uid) {
        const userUid = user.uid;
        businessVehicles = businessVehicles.filter(vehicle => {
          // Check multiple possible business identifier fields
          return (
            vehicle.businessUid === userUid || 
            vehicle.companyId === userUid ||
            vehicle.reservationSummary?.businessUid === userUid ||
            vehicle.reservationDetails?.businessUid === userUid ||
            vehicle.customer?.uid === userUid
          );
        });
      }
      
      setVehicles(businessVehicles);
    } catch (err) {
  ...
  const handleSendTrackingLink = async (e) => {
    ...
    try {
      ...
      if (contactMethod === 'phone' && !driverPhone.trim()) {
        throw new Error('Please enter driver phone number');
      }
      
      if (contactMethod === 'email' && !driverEmail.trim()) {
        throw new Error('Please enter driver email address');
      }

      // Prepare request data based on contact method
      const requestData = {
      ...
      if (contactMethod === 'phone') {
        requestData.phone = driverPhone;
      } else {
        requestData.email = driverEmail;
      }

      const response = await axios.post(`${API_BASE}/api/tracking/send-tracking-link`, requestData);
          ...
          <form onSubmit={handleSendTrackingLink} className="space-y-4">
            <div>
              ...
              >
                ...
                {vehicles.map(vehicle => (
                  <option key={vehicle.id || vehicle.truckId} value={vehicle.id || vehicle.truckId}>
                    {vehicle.number || `Vehicle ${vehicle.id || vehicle.truckId}`}
            ...
            {contactMethod === 'email' && (
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">We'll send the tracking link to this email address</p>
              </div>
            )}
                ...
                return (
                  <div key={session.sessionId} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      <div>
                        <span className="font-medium">Contact:</span> {formatPhoneNumber(session.driverPhone) || session.driverEmail || 'N/A'}
                      </div>
                      <div>
