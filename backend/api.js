...
app.post("/delivery/create", async (req, res) => {
  try {
    ...
    // basic validation
    if (!truckId || !origin || !destination || !eta) {
      return res.status(400).json({ success: false, error: "Missing truckId/origin/destination/eta" });
    }
    if (!payee) {
      return res.status(400).json({ success: false, error: "Missing payee address" });
    }
    if (!amount && amount !== 0) {
      return res.status(400).json({ success: false, error: "Missing amount" });
    }
    if (!ethers.isAddress(payee)) {
      return res.status(400).json({ success: false, error: `Invalid payee address: ${payee}` });
    }

    // eta should be a future unix timestamp (number)
    const etaNum = Number(eta);
    if (!Number.isFinite(etaNum) || etaNum <= Math.floor(Date.now() / 1000)) {
      return res.status(400).json({ success: false, error: "eta must be a future unix timestamp (seconds)" });
    }

    // 1️⃣ Create Delivery → returns orderId (stringified)
    const orderId = await createDelivery(truckId, origin, destination, etaNum);
    console.log(`✅ Delivery created with OrderId: ${orderId}`);

    // 2️⃣ Create Escrow (ETH) — createEscrowETH will convert amount to wei
    // amount should be an ether string/number, e.g. "0.1" or 0.1
    const escrowTx = await createEscrowETH(orderId, payee, amount);
    // thirdweb transaction object: be defensive
    const escrowHash = escrowTx?.receipt?.transactionHash ?? escrowTx?.hash ?? null;
    console.log(`✅ Escrow created: ${escrowHash}`);

    // 3️⃣ Initialize Proof of Delivery
    const podTx = await initProof(orderId);
    const podHash = podTx?.receipt?.transactionHash ?? podTx?.hash ?? null;
    console.log(`✅ Proof initialized: ${podHash}`);
    ...
    // If err.message contains "invalid address" or similar show it directly for easier debugging
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    let users = [];
    if (email) {
      users = await usersRepo.findWhere([
...
app.get("/api/business-profiles", async (req, res) => {
  try {
    ...
    if (postalBranch) {
      conditions.push({
        field: "businessInfo.postalBranchName",
        operator: "==",
        value: postalBranch,
      });
    }
...
app.get("/api/driver-profiles", async (req, res) => {
  try {
    ...
    if (stateOfIssue) {
      conditions.push({
        field: "licenseInfo.stateOfIssue",
        operator: "==",
        value: stateOfIssue,
      });
    }
...
app.get("/api/drivers/available", async (req, res) => {
  try {
    ...
    const [completedDrivers, approvedDrivers, pendingDrivers, noStatusDrivers] =
      await Promise.all([
    ...
    const availableDrivers = await Promise.all(
      allDriverProfiles.map(async (profile) => {
        ...
        const driver = {
          ...
          licenseNumber:
            profile.licenseInfo?.licenseNumber || profile.licenseNumber,
          licenseType: profile.licenseInfo?.licenseType,
          experience:
            profile.professionalInfo?.experience || profile.experience || 0,
          location: {
          ...
          walletAddress:
            ...
            profile.personalInfo?.walletAddress ||
            null, // ✅ Added here
        };
        ...
        if (includeSentiment === "true") {
          try {
            const sentimentData =
              await sentimentAnalysisService.getCurrentSentiment(profile.id);
            ...
            console.warn(
              `Failed to get sentiment for driver ${profile.id}:`,
              error.message
            );
            driver.sentiment = {
              score: null,
...
app.patch("/api/drivers/:id/assignment", async (req, res) => {
  try {
    ...
    const updatedProfile = await driverProfilesRepo.update(
      driverId,
      updateData
    );
...
app.post("/api/sentiment/survey", async (req, res) => {
  try {
    ...
    const result = await sentimentAnalysisService.processSurvey(
      driverId,
      surveyData
    );
    // Recompute gate after save to reflect current count
    const postGate = await sentimentAnalysisService.canSubmitSurvey(
      driverId,
      2
    );
    res.status(201).json({
...
app.get("/api/sentiment/driver/:id", async (req, res) => {
  try {
    ...
    const sentimentHistory =
      await sentimentAnalysisService.getDriverSentimentHistory(
        driverId,
...
app.get("/api/sentiment/driver/:id/quota", async (req, res) => {
  try {
    ...
    const quotaInfo = await sentimentAnalysisService.canSubmitSurvey(
      driverId,
      2
    );
...
app.get("/api/sentiment/driver/:id/current", async (req, res) => {
  try {
    ...
    const currentSentiment = await sentimentAnalysisService.getCurrentSentiment(
      driverId
    );
...
app.get("/api/trucks/enhanced", async (req, res) => {
  try {
    ...
    if (includeReserved === "true") {
      if (businessUid) {
        reservations = await truckReservationRepo.findByBusinessUid(
          businessUid
        );
      } else {
      ...
      reservedTrucks = reservations
        ...
        .map((reservation) =>
          TruckDataTransformers.reservationToTruck(reservation)
        );
    }
    ...
    for (const reservedTruck of reservedTrucks) {
      const exists = combinedTrucks.some(
        (truck) => truck.id === reservedTruck.id
      );
      if (!exists) {
    ...
    if (includeSentiment === "true") {
      for (let truck of combinedTrucks) {
        if (truck.driver?.id) {
          try {
            const sentimentData =
              await sentimentAnalysisService.getCurrentSentiment(
                truck.driver.id
              );
            if (sentimentData.hasSentimentData) {
              truck.driver = {
            ...
            console.warn(
              `Failed to get sentiment for driver ${truck.driver.id}:`,
              error.message
            );
          }
        }
...
app.post("/api/trucks/:id/reserve", async (req, res) => {
  try {
    ...
    const updatedTruck = TruckDataTransformers.reservationToTruck(
      reservation,
      truck
    );
...
app.post("/api/trucks/:id/load-unload", async (req, res) => {
  try {
    ...
    const newCurrentLoad =
      (truck.currentLoad || 0) -
      Number(unloadAmount || 0) +
      Number(loadAmount || 0);
...
app.get("/api/locations/search", async (req, res) => {
  try {
    ...
    const suggestions = await gpsTrackingService.getLocationSuggestions(
      query,
      options
    );
...
app.post("/api/reservations", async (req, res) => {
  try {
    const { businessUid, customerInfo, trucks, deliveryStatus, blockchainOrderId } = req.body;
    ...
    const reservationData = {
      ...
      trucks,
      deliveryStatus,
      blockchainOrderId, // ✅ add blockchain order ID
...
app.patch("/api/reservations/:id/status", async (req, res) => {
  try {
    ...
    const updatedReservation = await truckReservationRepo.updateStatus(
      reservationId,
      status
    );
...
app.get("/api/reservations/upcoming/pickups", async (req, res) => {
  try {
    ...
    const upcomingPickups = await truckReservationRepo.getUpcomingPickups(
      Number(days)
    );
...
app.get("/api/business/:uid/dashboard", async (req, res) => {
  try {
    ...
    const reservations = await truckReservationRepo.findByBusinessUid(
      businessUid
    );
    ...
    const deliveryStatus = {
      ...
      "in-progress": reservations.filter((r) => r.status === "in-progress")
        .length,
    ...
    const trucksInUse = activeReservations.reduce(
      (count, res) => count + (res.trucks?.length || 0),
      0
    );
  ...
  try {
    ...
    const stats = {
      fleet: {
        ...
        maintenanceTrucks,
        utilizationRate:
          totalTrucks > 0 ? ((activeTrucks / totalTrucks) * 100).toFixed(1) : 0,
      },
      reservations: {
        totalReservations,
        pendingReservations,
